...
CREATE TABLE ttelefoni (
    ...
    fk_contatto  BIGINT NOT NULL, 
    fk_operatore INT NOT NULL,
    PRIMARY KEY(id_telefono),
    INDEX itelefoni (numero),
    FOREIGN KEY(fk_contatto) REFERENCES tcontatti(id_contatto) /* Integrità refernziale -> sono certo che la chiave primaria dell'altra tabella esiste veramente*/
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    FOREIGN KEY(fk_operatore) REFERENCES toperatori(id_operatore)
        ON UPDATE CASCADE
        ON DELETE RESTRICT
) ENGINE = InnoDB;
...
CREATE TABLE tappartiene (
    ...
    PRIMARY KEY(fk_contatto, fk_gruppo),
    FOREIGN KEY(fk_contatto) REFERENCES tcontatti(id_contatto) 
        ON UPDATE CASCADE 
        ON DELETE CASCADE,
    FOREIGN KEY(fk_gruppo) REFERENCES tgruppi(id_gruppo)
        ON UPDATE CASCADE 
        ON DELETE CASCADE
) ENGINE = InnoDB;
